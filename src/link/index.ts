...
export default async function (
  ...
  const resolvePeersResult = await resolvePeers(tree, rootNodeIds, topPkgIds, opts.topParents, opts.independentLeaves, opts.baseNodeModules)
  ...
  const rootPkgsToLink = flatResolvedDeps
    .filter(pkg => pkg.depth === 0)
    .reduce((rootPkgsToLink, pkg) => {
      rootPkgsToLink[pkg.absolutePath] = pkg
      return rootPkgsToLink
    }, {})
  for (let rootAlias of R.keys(resolvePeersResult.rootResolvedIds)) {
    const pkg = rootPkgsToLink[resolvePeersResult.rootResolvedIds[rootAlias]]
    if (!pkg) continue
...
async function linkAllBins (
  ...
  return Promise.all(
    pkgs.map(dependency => limitLinking(async () => {
      ...
      await Promise.all(
        R.props<DependencyTreeNode>(R.values(childrenToLink), pkgMap)
